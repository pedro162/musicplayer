...
import {View, Text, FlatList, TextInput, TouchableOpacity,ProgressBarAndroid, Alert} from 'react-native';
...
//clonando === true ? (<><Progresso color={'#FFF'} passo={false} type={false} indeterminate={1} /></>)


const Pesquisar = ({navigation, procurado, pesquisar, setPesquisar, setItemLooked, data, setLoading, setError, setAallItemsMout,setItem, setModalVisible, filters})=>{
	/*
		procurado={itemLook}
	...
	*/
	...
	//sincronizar();
	const [clonando, setClonando] = React.useState(null)
	const [editando, setEditanto] = React.useState(null)
	const {setIdVenda,atualizaDataVenda, idPessoaVendedor, nrVendedor, idUsuario} = React.useContext(DataContex);
...
//
	...
	//---- clona o registro ------
	const editar = async (idVenda)=>{
		try{
			let err = [];
			if(! (idVenda && idVenda > 0)){
				err.push('Parametro inválido. Entre em contato com o suporte')
			}
			setEditanto(true)
			const dataVend = await findeOneVenda({id:idVenda}); 
			if(! dataVend){
				err.push('Venda não identificada')
			}

			if(dataVend && dataVend.hasOwnProperty('status') && String(dataVend.status).trim() != 'registro'){
				err.push('Só se pode editar venda com status "Registro"')
			}
			if(err.length > 0){
			...
			atualizaDataVenda();
			navigation.navigate("PdvMenu", {screen: 'Pdv'});
	...
	const clonar = async (idVenda)=>{
  		...
  		try{
			...
			const dataClone = await findeOneVenda({id:idVenda}); 
			const dataVendProd = await getVendaProduto({venda_id:idVenda});
			console.log('------------------------------- Clonar =========================================================')
			console.log(dataVendProd)
			console.log('------------------------------- Clonar =========================================================')
			...
			let dataToSave={
				...
				obs_nf:null,
				status:'registro',
				pessoa_vendedor_id:idPessoaVendedor,
				ativo:'yes',
				usuario_id:idUsuario,
				id_api:null,
				nf_id:null,
				vr_bruto: dataClone?.vr_bruto,
				vr_liquido: dataClone?.vr_liquido,
				vr_desconto: dataClone?.vr_desconto,
				tp_desconto: dataClone?.tp_desconto,
				endereco_entrega_id: dataClone?.endereco_entrega_id,
				pessoa_cancel_id:null,
				solicita_desconto:null,
				aprova_recusa_desconto:null,
				pessoa_libera_desconto_id:null,
				filial_id: dataClone?.filial_id,
				pessoa_id: dataClone?.pessoa_id,
				rca_vendedor:nrVendedor,
			...
			if(retorno && retorno.hasOwnProperty('id') && Number(retorno.id) > 0){
				...
				if(Array.isArray(dataVendProd) && dataVendProd.length > 0){
					for(let i=0; !(i == dataVendProd.length); i++){
						let atual = dataVendProd[i];
						

						let lastVendProd = await findeLastVendaProduto();
						let idVendProdNext = 1;
						if(lastVendProd && lastVendProd.hasOwnProperty('id') && Number(lastVendProd.id) > 0 ){
							idVendProdNext = Number(lastVendProd.id) + 1;
						}

						let dataProdutoToSave ={	
							id:idVendProdNext,
							status: atual?.status,
							tp_desconto: atual?.tp_desconto,
							ativo:'yes',
							dt_cincronizacao: atual?.dt_cincronizacao,
							vr_bruto: atual?.vr_bruto,
							vr_desconto: atual?.vr_desconto,
							vr_liquido: atual?.vr_liquido,
							qtd_devolvido: null,
							qtd: atual?.qtd,
							embalagem_id: atual?.embalagem_id,
							usuario_id: idUsuario,
							venda_id: retorno.id,
						...
						let retornoVendProd = await connection.getRepository('VendasProdutos').save(dataProdutoToSave);
									
						console.log('------------------ Produto adicionado  ---------------------')
						console.log(retornoVendProd)
						console.log('------------------ Produto adicionado  ---------------------')//
			...
			if(allClients && Array.isArray(allClients) && allClients.length > 0){
				...
				for(let i=0; !(i == allClients.length); i++){
					...
					let acoes_right_row =[
						...
						{
							label:<View style={{flexDirecion:'column'}}><FontaWesome5 name='file-alt' color="#FFF" size={26} /><Text style={{fontSize:8, color:'#FFF'}}>Ver</Text></View>,
							action:()=>navigation.navigate("PdvMenu",{screen: 'Ver',params:{'venda_id':atual.id}}),
						...
						{
							label:<View style={{flexDirecion:'column'}}><FontaWesome5 name='copy' color="#FFF" size={26} /><Text style={{fontSize:8, color:'#FFF'}}>Clonar</Text></View>,
							action:()=>clonar(atual.id),
							propsLabel:{style:{ fontSize: 17, color: '#fff', paddingRight:10, paddingLeft:10,}},
					...
					if(String(atual.status) == 'orcamento'){

					}else if(String(atual.status) == 'registro'){

						acoes_right_row.unshift(
							{
						...
						acoes_right_row.unshift(
							{
								label:<View style={{flexDirecion:'column'}}><FontaWesome5 name='trash' color="#FFF" size={26} /><Text style={{fontSize:8, color:'#FFF'}}>Excluir</Text></View>,
								action:()=>Alert.alert('Clonar: '+atual.id),
								propsLabel:{style:{ fontSize: 17, color: '#fff', paddingRight:10, paddingLeft:10,}},
								propsBoxLabel:{style:{backgroundColor: '#FF0000', justifyContent:'center'}}
							}
						)
					}else if(String(atual.status) == 'prevenda'){

					}else if(String(atual.status) == 'pedido'){
						...
						cols:[
							{
								label:atual.id,
								props_col:{
									style:{maxWidth:'15%',color: '#000', minWidth:'15%',padding:10, flexDirecion:'column',backgroundColor:'',}
								},
								hasActionCol:true,
								actionCol:()=>null,
							},
	...
	React.useEffect(()=>{
		...
		const loadVendaProdutoData = async ()=>{
	...
	React.useEffect(()=>{
		...
		const loadProdPdvaData = async ()=>{
			try{
				setLoading(true)
				let paramFilter = {...filters}
				if(procurado && procurado.trim().length > 0){
					if(!isNaN(Number(procurado))){
						paramFilter.id=procurado
					}else{
				...
				const retornoVendas = await getVenda(paramFilter);//getProduto
				console.log('Dados das vendas -------------------'+(Math.random(100) +1))
				console.log(retornoVendas)
				
				prepareDataClientToShow(retornoVendas)	

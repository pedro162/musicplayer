...
import {View, Text, FlatList, TextInput, TouchableOpacity,ProgressBarAndroid} from 'react-native';
...
//
	...
	/*const getEmbProd = async ({nome, tp_embalagem, produto_id})=>{
		const connection = getConnection();
	...
	React.useEffect(()=>{
		...
		const loadProdPdvaData = async ()=>{
			try{
				...
				const retornoPessoa = await getEstoque(paramFilter);//getProduto
				console.log('Dados do estoque -------------------'+(Math.random(100) +1))
				console.log(retornoPessoa)
